...
TYPER.prototype = {
	...
	loadPlayerData: function() {
		...
		var p_name = prompt("Players name:");
	...
	generateWord: function() {
		...
		var random_index = (Math.random() * (this.words[generated_word_length].length - 1))
			.toFixed();
	...
	keyPressed: function(event) {
			...
			if (letter === this.word.left.charAt(0)) {
				...
				document.getElementById('score')
					.innerHTML = "Score: " + this.player.score;
...
function hardcoremode() {
	if (dM === 1) {
		document.body.style.backgroundImage = "url('46787593-black-picture.jpg')";
		console.log("Hardcore Mode OFF");
		dM = 0;
	} else {
		document.body.style.backgroundImage = "url('Matrix.gif-c200')";
		dM = 1;
		console.log("Hardcore Mode ON");
	}
}
...
function timer(time, showTime) {
	...
	r = setInterval(function() {
		...
		if (--timer < 0) {
			...
			var replay = confirm("Game over. Your score: " + typerGame.player.score + " Play again?");
			if (replay === true) {
				clearInterval(r);
				timer = time;
